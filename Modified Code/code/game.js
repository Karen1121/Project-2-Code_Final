  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      // Because there is a third case (space ' '), use an "else if" instead of "else"
        ...
        fieldType = "drag"; //A surface to slow down player
      else if (ch == "*")
        fieldType = "speeder"; //A surface to speed up player
...
function Portal4(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(2, 2.5);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
Portal4.prototype.type = "portal4";

function Portal5(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(2, 2.5);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
Portal5.prototype.type = "portal5";
...
Portal1.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Portal3.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

Portal4.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

Portal5.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  else if (type != "drag" && type != "speeder") {
    playerXSpeed = 7;
    jumpSpeed = 20;
  }
  if (type == "portal1") {
    this.player.pos = new Vector(14, 16);
    playerXSpeed = 7;
    jumpSpeed = 20;
  }
  if (type == "portal2") {
    this.player.pos = new Vector(54, 5);
    playerXSpeed = 7;
    jumpSpeed = 20;
  }
  if (type == "portal3") {
    this.player.pos = new Vector(8, 2);
    playerXSpeed = 7;
    jumpSpeed = 20;
  }
  if (type == "portal4") {
    this.player.pos = new Vector(71, 28);
    playerXSpeed = 7;
    jumpSpeed = 20;
  }
  if (type == "portal5") {
    this.player.pos = new Vector(5, 7);
    playerXSpeed = 7;
    jumpSpeed = 20;
  }
};
